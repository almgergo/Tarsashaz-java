...
public class Backlog implements HasDate {
	...
	public void updateInterest(Calendar paymentDate) {
		...
		// System.out.println("reqDate: " + this.date.getTime() + ", payDate: " +
		// payment.getDate().getTime() + ", delta: "
		// + dayDelta);
	...
	private void setInterest(double rate, int dayDiff) {
		if (this.interestRate != rate && rate > this.interestRate) {
			double oldSurcharge = this.surcharge;
			double newSurcharge = this.surcharge + Math.round((rate - this.interestRate) * this.requiredPayment);
			this.surcharge += newSurcharge;

			double oldInterestRate = this.interestRate;
			double newInterestRate = rate;
			// System.out.println("Interest increased from: " + oldInterestRate + " to: " +
			// newInterestRate
			// + ", surcharge increased by: " + (newSurcharge - oldSurcharge));
